...
public sealed class CombatRoundReportsDecoder : IFieldDecoder
{
    ...
    public DecodedFieldValue Decode(ref FieldDecodeContext context, FBitArchive archive)
    {
        // The owner of every report in this payload is the actor the
        // CombatReportComponent lives on — an AOwnerExclusivePlayerInfo. Its
        // netguid is the actor netguid of the export group. The struct itself
        // does NOT carry an owner field (confirmed against the SDK dump and the
        // net-field export group, which only has participant-side fields).
        var ownerPlayerState = context.ActorNetGuid.IsValid
            ? (uint?)context.ActorNetGuid.Value
            : null;
        var reports = ReadArray(archive, (index, item) => ReadRound(index, item, ownerPlayerState));
        archive.EnsureFullyConsumed(nameof(CombatRoundReportsDecoder));
        return DecodedFieldValue.FromObject(reports);
    }

    private static CombatRoundReportUpdate ReadRound(int index, FBitArchive archive, uint? ownerPlayerState)
    {
        ...
        return new CombatRoundReportUpdate(index, roundNumber, reports, ownerPlayerState);
    }
