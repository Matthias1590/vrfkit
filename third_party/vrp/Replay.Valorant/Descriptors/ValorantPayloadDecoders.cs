...
internal static class ValorantPayloadDecoders
{
    ...
    // FVector_NetQuantize: 7-bit packed header + N-bit signed components, scale = 1.
    // Used by DamageImpactLocation and DamageImpactBoneRelativeLocation.
    public static IFieldDecoder VectorNetQuantize(string typeName) =>
        PrimitiveOrRaw(PrimitiveDecoders.VectorNetQuantize, typeName);

    // FVector_NetQuantize100: packed header + signed components, scale = 100.
    // Used by DamageOrigin.
    public static IFieldDecoder VectorNetQuantize100(string typeName) =>
        PrimitiveOrRaw(PrimitiveDecoders.VectorNetQuantize100, typeName);

    // FVector_NetQuantizeNormal: 3 x 16-bit fixed normalized components (unit vector).
    // Used by DamageDirection and DamageImpactNormal.
    public static IFieldDecoder VectorNetQuantizeNormal(string typeName) =>
        PrimitiveOrRaw(PrimitiveDecoders.VectorNetQuantizeNormal, typeName);
