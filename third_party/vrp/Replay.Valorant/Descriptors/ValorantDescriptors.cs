...
using Replay.Valorant.Descriptors.Effects;
using Replay.Valorant.Descriptors.Effects.Replay;
using Replay.Valorant.GameState;
using Replay.Valorant.Objectives;
using Replay.Valorant.Weapons;
...
public static class ValorantDescriptors
{
    public static DescriptorCatalog CreateCatalog()
    {
        ...
        // --- valplay extensions: previously-undecoded groups (build 13.01) ---
        // Objective: the spike (plant site, defuse progress, defuser, time-to-explode).
        catalog.Add(new TimedBombDescriptor());
        // Economy / purchases.
        catalog.Add(new PurchasedItemComponentDescriptor());
        catalog.Add(new OwnerExclusivePlayerInfoDescriptor());
        // Inventory / dropped weapons.
        ...
        // Small component descriptors.
        catalog.Add(new AbilityTrackingDelegateComponentDescriptor());
        catalog.Add(new AutoEquipTransitionContextDescriptor());
        catalog.Add(new AresEquippableDataTrackerDescriptor());
        // Active gameplay effects (smokes, walls, flashes) + their multicast RPCs.
        catalog.Add(new EffectManagerComponentDescriptor());
        catalog.Add(new EffectManagerComponentClassNetCacheDescriptor());
        // Weapon actor bodies (Vandal, Phantom, Ghost, melee, etc.). Carry the
        // replicated cosmetic/loadout shell of each weapon (skin, chroma, charm,
        // attach bookkeeping). Per-weapon gameplay state lives on components.
        catalog.Add(WeaponEquippableDescriptors.CreateDescriptors());
