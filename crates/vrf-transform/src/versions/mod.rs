...
//!
//! In all seven, `TAIL_XOR == SEED_ADDEND & 0xff`. That is asserted per version
...
//! ## One file per build
//!
//! The seven `impl`s are deliberately kept in separate files. They are near-
...
mod tests {
    ...
    /// Adding the offset is the exception, not the rule: 12.11 and 13.05 are the
    /// only builds that do it. Pinning the exact set means a new transform that
    /// copy-pastes the wrong sign is caught here rather than in a corpus sweep.
    #[test]
    fn only_12_11_and_13_05_add_the_offset() {
